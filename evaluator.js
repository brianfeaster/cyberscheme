...
onmessage = function (m) {
  const newWorker = (code)=>new Worker(URL.createObjectURL(new Blob([code], {type: "text/javascript"})));
  scanner = newWorker(m.data);
  onmessage = function (m) {
    parser = newWorker(m.data);
    onmessage = dispatch;
  };
};
